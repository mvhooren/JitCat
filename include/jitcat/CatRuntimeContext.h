...
public:
	...
	CatScopeID addScope(TypeInfo* typeInfo, Reflectable* scopeObject = nullptr, bool isStatic = false);
	//Same as addScope but uses a custom type (a struct defined at runtime) instead of a reflected C++ class.
	CatScopeID addCustomTypeScope(CustomTypeInfo* typeInfo, CustomTypeInstance* scopeObject = nullptr, bool isStatic = false);
	...
	void setScopeObject(CatScopeID id, Reflectable* scopeObject);
	//Returns weither or not the provided ScopeID is a static scope.
	bool isStaticScope(CatScopeID id) const;
	
	Reflectable* getScopeObject(CatScopeID id) const;
	TypeInfo* getScopeType(CatScopeID id) const;
	...
	TypeMemberInfo* findVariable(const std::string& lowercaseName, CatScopeID& scopeId);
	MemberFunctionInfo* findFunction(const std::string& lowercaseName, CatScopeID& scopeId);

	LLVMCodeGenerator* getCodeGenerator() const;
	int getNextFunctionIndex();

private:
	CatScopeID createScope(Reflectable* scopeObject, TypeInfo* type, bool isStatic);
