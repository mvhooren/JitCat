...
inline bool ExpressionAssignment<ExpressionT>::assignInterpretedValue(CatRuntimeContext* runtimeContext, typename TypeTraits<ExpressionT>::functionParameterType value)
{
	if (expressionAST != nullptr && expressionAST->isAssignable())
	{
		...
		if constexpr (TypeTraits<ExpressionT>::isReflectableType())
		{
			ASTHelper::doAssignment(target, std::any(static_cast<Reflectable*>(value)), getExpectedCatType(), assignableType);
		}
		else
		{
			ASTHelper::doAssignment(target, std::any(value), getExpectedCatType(), assignableType);
		}
		return true;
	}
	return false;
